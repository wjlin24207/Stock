...
target_stocks = ["^TWII", "0056", "00878", "00919", "0050"]
inventory_stocks = ["^TWII", "2454"]  # ✅ 加入庫存股

# ✅ 合併
all_stocks = list(set(target_stocks + inventory_stocks))
...
for sid in all_stocks:   # ✅ 這裡改
    ...
    if not live:
        live = {"z": "-", "y": hist['Close'].iloc[-1] if hist is not None else 0}
...
st.subheader("📌 自選股監控")
df_watch = df[df["代號_raw"].isin(target_stocks)]
st.dataframe(df_watch, use_container_width=True)

st.subheader("💼 庫存明細")

# ✅ 這是關鍵
df_inventory = df[df["代號_raw"].isin(inventory_stocks)]
